...
void DBSevaTypeViewModelFirebase::processSevaList()
{
    QVariantMap :: Iterator it2;
    for(it2=p_sevaListVariantmap.begin();it2!=p_sevaListVariantmap.end();it2++)
    {
        p_keyvalues = it2.key();
        qDebug()<<"Keyssssssssssssssssssssssssssssssssssssssssss"<<p_keyvalues;
        if(p_keyvalues==p_sevaListHit)
        {

            qDebug()<<"Hittt in db-ITvalue-catchdatafomdbdeva2"<<it2.value();
            p_sevanameslistdata=it2.value().toStringList();
            qDebug()<<"sringlists00-0-"<<p_sevanameslistdata;
